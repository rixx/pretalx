...
def test_default_social_preview_settings(event):
    ...
    assert event.social_preview_settings["submission"]["enabled"] is True
    assert event.social_preview_settings["submission"]["layout"] == "default"
    assert event.social_preview_settings["speaker"]["enabled"] is True
...
def test_generate_submission_preview_minimal_layout(event, submission):
    ...
    settings_dict = {"layout": "minimal", "enabled": True, "show_event_name": True}
...
def test_generate_submission_preview_full_layout(event, submission):
    ...
    settings_dict = {"layout": "full", "enabled": True, "show_event_logo": True}
    img = generate_submission_preview(event, submission, settings_dict)
    assert img is not None
    assert img.size == (1200, 630)
...
@pytest.mark.django_db
def test_generate_submission_preview_disabled(event, submission):
    """Test that no image is generated when disabled."""
...
@pytest.mark.django_db
def test_generate_speaker_preview_disabled(event, speaker):
    """Test that no image is generated when disabled."""
    settings_dict = {"enabled": False}
    img = generate_speaker_preview(event, speaker, settings_dict)
    assert img is None
...
def test_settings_hash_generation():
    ...
    settings1 = {"layout": "default", "enabled": True}
    settings2 = {"layout": "default", "enabled": True}
    settings3 = {"layout": "minimal", "enabled": True}
...
def test_multiple_layouts_different_caches(event, submission):
    ...
    settings_default = {"layout": "default", "enabled": True}
    settings_minimal = {"layout": "minimal", "enabled": True}
