...
@pytest.mark.django_db
def test_api_submission_orga_change_request_field(client, orga_user_write_token, submission):
    with scope(event=submission.event):
...
@pytest.mark.django_db
def test_api_accept_change_request(client, orga_user_write_token, submission):
    with scope(event=submission.event):
        old_title = submission.title
    ...
    url = submission.event.api_urls.submissions + f"{submission.code}/accept-change-request/"
    response = client.post(
        url,
        follow=True,
        headers={"Authorization": f"Token {orga_user_write_token.token}"},
...
@pytest.mark.django_db
def test_api_accept_change_request_none(client, orga_user_write_token, submission):
    url = submission.event.api_urls.submissions + f"{submission.code}/accept-change-request/"
    response = client.post(
        url,
        follow=True,
...
@pytest.mark.django_db
def test_api_reject_change_request(client, orga_user_write_token, submission):
    with scope(event=submission.event):
        old_title = submission.title
    ...
    url = submission.event.api_urls.submissions + f"{submission.code}/reject-change-request/"
    response = client.post(
        url,
        follow=True,
        headers={"Authorization": f"Token {orga_user_write_token.token}"},
...
@pytest.mark.django_db
def test_api_reject_change_request_none(client, orga_user_write_token, submission):
    url = submission.event.api_urls.submissions + f"{submission.code}/reject-change-request/"
    response = client.post(
        url,
        follow=True,
